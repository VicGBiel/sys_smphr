...
void ssd1306_circle(ssd1306_t *ssd, uint8_t xc, uint8_t yc, uint8_t r, bool value) {
  ssd1306_fill_circle(ssd, xc, yc, r, value);         // círculo maior (externo)
  ssd1306_fill_circle(ssd, xc, yc, r - 1, !value);    // "apaga" o centro, criando o contorno
}

// Desenha um circulo preenchido
void ssd1306_fill_circle(ssd1306_t *ssd, uint8_t xc, uint8_t yc, uint8_t r, bool value) {
  for (int y = -r; y <= r; y++) {
    for (int x = -r; x <= r; x++) {
      if (x * x + y * y <= r * r) {
        ssd1306_pixel(ssd, xc + x, yc + y, value);
      }
    }
  }
}
